---
title: リレイヤーログ  
description: リレイヤーのログとアクティビティを確認する  
updated: 2024-05-31  
...
リレイヤーの動作を分析するために、**リレイヤーログを確認** できます。Avalanche CLI を使用している場合、次のコマンドでアクセスできます。  

```bash
avalanche teleporter relayer logs
```

また、**JSON 形式のログファイル** は `~/.avalanche-cli/runs/awm-relayer.log` に保存されています。このファイルを開いて内容を確認するには、次のコマンドを実行してください。  
...
リレイヤーログには、さまざまな情報が記録されています。特に重要なポイントを解説します。  

- **`Initializing awm-relayer`** → AWM リレイヤーの初期化  
- **`Set config options`** → 設定オプションの読み込み  
- **`Initializing destination clients`** → 宛先ブロックチェーン（L1）のクライアントを初期化  
- **`Initialized destination client`** → 宛先ブロックチェーンへの接続が完了  

この時点でリレイヤーが正常に動作を開始したことが確認できます。  

---

## **メッセージのリレー処理**  

リレイヤーが**メッセージを送信** する際のログは次のようになります。  
...
1. **`Unpacked warp message`** → 送信されたメッセージを解析  
2. **`Fetching aggregate signature`** → 送信元のバリデータから集約署名を取得  
3. **`Created signed message`** → 署名されたメッセージを作成  
4. **`Sending message to destination chain`** → 宛先のブロックチェーンへメッセージを送信  
5. **`Sent transaction`** → 宛先の L1 にトランザクションを送信  

これにより、メッセージが正常に転送されたことが確認できます。  

---

## **メッセージ配信の完了**  

リレーが**正常に完了** すると、次のようなログが出力されます。  
...
- **`Delivered message to destination chain`** → メッセージが正常に送信された  
- **`Finished relaying message`** → リレー処理が完了  

このようなログが出力されていれば、**メッセージが無事に送信された** ことが確認できます。
以下がログ全体になります。
